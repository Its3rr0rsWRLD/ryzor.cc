  ...
  try {
    ...
    switch (action) {
      case "start_backup":
        ...
        const { count, canCreate, oldestBackup } = await BackupManager.checkBackupLimit(
          discordUser.id
        );
          ...
          return NextResponse.json(
            {
              ...
              message:
                "You have reached the maximum of 3 backups. The oldest backup will be deleted to create a new one.",
              oldestBackup: {
                id: oldestBackup.id,
      ...
      case "confirm_backup_with_deletion":
        ...
        const { count: confirmCount, oldestBackup: confirmOldest } =
          await BackupManager.checkBackupLimit(discordUser.id);
      ...
      case "restore_backup":
        ...
        try {
          ...
          const failedProfileChange = restoreResult.changes.profile.find(
            (c: any) => c.status === "failed" && c.error && c.error.includes("CapSolver key")
          );
          ...
          if (
            err.message &&
            (err.message.includes("CapSolver") ||
              err.message.includes("proxy") ||
              err.message.includes("proxies") ||
              err.message.includes("No proxies set"))
          ) {
    ...
    return NextResponse.json(
      { ok: false, error: "Unknown server error (no response returned)" },
      { status: 500 }
    );
  } catch (e: any) {
...
async function downloadUserMedia(profile: any, backupId: string) {
  ...
  try {
    ...
    if (profile.banner) {
      const bannerUrl = `https://cdn.discordapp.com/banners/${profile.id}/${profile.banner}.${
  ...
  try {
    ...
    try {
      const gifsResponse = await fetch(
        "https://discord.com/api/v9/gifs/search/trending?provider=tenor&locale=en-US&media_format=gif",
        {
          headers: { Authorization: token },
        }
      );
...
async function performRestore(
  ...
  try {
    ...
    if (backupData.profile && (backupData.type === "full" || backupData.type === "settings")) {
      const profileChanges = await compareAndRestoreProfile(
        backupData.profile,
        currentProfile,
        token,
        solverKey,
    ...
    if (backupData.servers && (backupData.type === "full" || backupData.type === "servers")) {
      const serverChanges = await compareAndRestoreServers(
        backupData.servers,
        currentServers,
        token
      );
    ...
    if (backupData.settings && (backupData.type === "full" || backupData.type === "settings")) {
      const settingsChanges = await compareAndRestoreSettings(
        backupData.settings,
        currentSettings,
        token
      );
...
async function compareAndRestoreProfile(
  backupProfile: any,
  currentProfile: any,
  token: string,
  solverKey?: string,
  ...
  try {
    if (backupProfile.globalName && backupProfile.globalName !== currentProfile.globalName) {
      const updateResult = await updateUserProfile(
        token,
        { global_name: backupProfile.globalName },
        solverKey,
        userProxies
      );
    ...
    if (backupProfile.pronouns && backupProfile.pronouns !== currentProfile.pronouns) {
      const updateResult = await updateUserProfile(
        token,
        { pronouns: backupProfile.pronouns },
        solverKey,
        userProxies
      );
      if (updateResult.success) {
    ...
    if (backupProfile.avatar && backupProfile.avatar !== currentProfile.avatar) {
      try {
        if (backupProfile.media && backupProfile.media.avatar && backupProfile.media.avatar.data) {
          const avatarData = `data:image/${
            backupProfile.media.avatar.filename.includes("gif") ? "gif" : "png"
          };base64,${backupProfile.media.avatar.data}`;
          const updateResult = await updateUserProfile(
            token,
            { avatar: avatarData },
            solverKey,
          ...
          const avatarUrl = `https://cdn.discordapp.com/avatars/${backupProfile.id}/${
            backupProfile.avatar
          }.${backupProfile.avatar.startsWith("a_") ? "gif" : "png"}?size=512`;
          const avatarResponse = await fetch(avatarUrl);
          if (avatarResponse.ok) {
            const avatarBuffer = await avatarResponse.arrayBuffer();
            const extension = backupProfile.avatar.startsWith("a_") ? "gif" : "png";
            const avatarBase64 = `data:image/${extension};base64,${Buffer.from(
              avatarBuffer
            ).toString("base64")}`;
            const updateResult = await updateUserProfile(
              token,
    ...
    if (backupProfile.banner && backupProfile.banner !== currentProfile.banner) {
      try {
        if (backupProfile.media && backupProfile.media.banner && backupProfile.media.banner.data) {
          const bannerData = `data:image/${
            backupProfile.media.banner.filename.includes("gif") ? "gif" : "png"
          };base64,${backupProfile.media.banner.data}`;
          const updateResult = await updateUserProfile(
            token,
            { banner: bannerData },
            solverKey,
          ...
          const bannerUrl = `https://cdn.discordapp.com/banners/${backupProfile.id}/${
            backupProfile.banner
          }.${backupProfile.banner.startsWith("a_") ? "gif" : "png"}?size=1024`;
          ...
          if (bannerResponse.ok) {
            ...
            const bannerBase64 = `data:image/${extension};base64,${Buffer.from(
              bannerBuffer
            ).toString("base64")}`;
            const updateResult = await updateUserProfile(
              token,
  ...
  try {
    const response = await fetch("https://discord.com/api/v9/users/@me", {
      ...
      headers: {
        ...
        "x-super-properties":
          "",
      },
    ...
    if (!response.ok) {
      ...
      if (errorDetail && errorDetail.captcha_key) {
        if (!solverKey) {
          throw new Error(
            "You need to set your CapSolver key in General/Settings to restore your profile."
          );
        }
        ...
        if (retryResponse.ok) {
          ...
          throw new Error(
            `Captcha solved but request still failed: ${retryError.message || "Unknown error"}`
          );
        }
      }
      throw new Error(
...
async function getProxy(userProxies?: string[]) {
  if (!userProxies || !Array.isArray(userProxies) || userProxies.length === 0) {
    throw new Error(
      "No proxies set. Please add proxies in General/Settings before restoring your profile."
    );
  }
...
async function solveCaptcha(captchaData: any, solverKey: string, userProxies?: string[]) {
  try {
    ...
    const createTaskResponse = await fetch("https://api.capsolver.com/createTask", {
      ...
      body: JSON.stringify({
        ...
        task: {
          ...
          userAgent:
            "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) discord/1.0.9194 Chrome/134.0.6998.205 Electron/35.3.0 Safari/537.36",
          isInvisible: false,
          proxy: proxy,
        },
      }),
    ...
    if (!createTaskResponse.ok) {
      throw new Error(
        `CapSolver API error: ${createTaskResponse.status} ${createTaskResponse.statusText}`
      );
    }
    ...
    if (createTaskResult.errorId !== 0) {
      if (
        createTaskResult.errorDescription &&
        createTaskResult.errorDescription.includes("proxy")
      ) {
        ...
        if (!fallbackResponse.ok) {
          throw new Error(
            `CapSolver API error (retry): ${fallbackResponse.status} ${fallbackResponse.statusText}`
          );
        }
        const fallbackResult = await fallbackResponse.json();
        if (fallbackResult.errorId !== 0) {
          throw new Error(
            `CapSolver create task error (after retry): ${fallbackResult.errorDescription}`
          );
        }
    ...
    while (attempts < maxAttempts) {
      ...
      if (!getResultResponse.ok) {
        throw new Error(
          `CapSolver get result API error: ${getResultResponse.status} ${getResultResponse.statusText}`
        );
      }
